	...
	render() {
		  return (
				...
				<div className="sidenav">
				  ...
				  }} className="btn btn-light">Recup vos infos</button></h6></center>

				  <center><h6><button onClick={() => {
				  	this.setState({current : 2});
				  	this.setState({makepdv : 0});
				  }} className="btn btn-light">Recup infos Medecin</button></h6></center>

				  <center><h6><button id="toomuch" onClick={() => {
				  	this.setState({current : 3});
				  	this.setState({makepdv : 0});
				  }} className="btn btn-light">Recup infos compagnie d'assurance</button></h6></center>

				  <center><h6><button onClick={() => {
				  	this.setState({current : 4});
				  	this.setState({makepdv : 0});
				  }} className="btn btn-light">Demander  rembours.</button></h6></center>
				</div>
				{
					(this.state.current==1)
					?    (<form id="gpd" className="abc">	
					  <div className="form-group">
					    <label htmlFor="exampleInputEmail1">Numero Carte vital</label>
					    <input type="number" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter votre Carte vital"></input>
					 </div>
					  <div className="aab">
					  <button onClick={async (event) => {
						event.preventDefault();
							...
							const result = await contract.methods.getPatientInfo(this.state.pacn).call({ from: accounts[0] });
			    ...
			    {	
					...
					?    (<form id="gic" className="abc">	
					  <div className="form-group">
					    <label htmlFor="exampleInputEmail1">ID assurance</label>
					    <input type="email" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter ID assurance"></input>
					 </div>
					  <div className="aab">
					  <button onClick={async(event) => {
					  	event.preventDefault();
			    ...
			    }
			    {
					...
					?    (<form id="afi" className="abc">	
					  <div className="form-group">
					    <label htmlFor="exampleInputEmail1">Numero Carte vital</label>
					    <input type="email" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter votre Carte vital "></input>
					 </div>
					  <div className="aab">
					  <button onClick={async(event) => {
					  	event.preventDefault();
